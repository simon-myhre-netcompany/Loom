---
name: loom-logg
description: Use Loom (the `loom` CLI) to search the user's recent work history across all the apps it integrates (Tempo, Jira, Confluence, GitHub, Slack, Apple Mail, Apple Calendar), then help them either fill Tempo (Loom can WRITE worklogs — but discuss before writing) or draft their weekly status (ukentlig status). Use when the user says "loom logg", "logg", asks what to fill in Tempo, asks for a weekly status / ukentlig status draft, or asks what they worked on.
---

# loom-logg

**Loom** is a CLI (`loom`) that integrates across the apps the user works in and
is **built for an AI (you) to drive** — so you can pull their real work history
yourself instead of them copy-pasting it. Apps it integrates:
...
- **tempo** — logged hours. **Read and write**: create worklogs (`tempo log`),
  ...
  (`tempo set-account`).
- **jira** — issues + comments (the user's, or **everyone's** with `--all`).
  Also **guarded writes**: `comment`, `transition` (status), `describe`,
  `estimate`, `assign`, `rename`, `labels`, `set` (priority/due).
  **Closing/resolving a ticket**: the workflow requires extra screen fields —
  ...
  `loom jira transition --key ABC-123 --to Resolved --resolution Fixed
  --field "Løsningsmetode=Oppdaterte endepunkter i prod og test" --dry-run`
- **confluence** — pages the user edited, incl. their weekly-status page.
...
and acts as the authenticated user — **never write before the user agrees.**

## Step 1 — always start by searching their history with Loom
...
```bash
loom tempo worklogs --since 3w --json     # what is already logged (+ the user's style)
...
Then help the user with **one of two things**:

## A) Fill Tempo (Loom can write — but discuss first)

The goal is to log their hours. Loom *can* write to Tempo, but **never write
before the user agrees.** Flow:

1. **Learn their logging style** from the **previous 3 weeks** of worklogs
   (`loom tempo worklogs --since 3w --json`): which issues they log to, how they
   phrase descriptions, how hours are distributed across the day. Mirror that.
2. **Search what they actually did** in the target period from the other apps
   (Jira/GitHub/calendar/Slack/mail) and group it by likely Tempo issue.
3. **Ask the user how many hours they worked each day** in the period — don't
   guess the total. Build the plan around their answer.
4. **Propose a per-day breakdown**: issue + hours + short description per entry,
   in their style. **Hard constraint: max 2.5 hours per worklog.** Split longer
   work into multiple entries (across issues, or the same issue with distinct
   descriptions) so no single entry exceeds 2.5h.
5. **Discuss and iterate** until they're happy with the breakdown.
6. **Only then write**, one entry at a time:

   ```bash
   loom tempo log --issue ABC-123 --hours 2 --date 2026-06-09 \
     --description "..." --yes
   ```

   (`--issue` takes a Jira key or numeric id; `--date` defaults to today,
   `--start HH:mm` defaults to 09:00. Without `--yes` it asks to confirm.)
   Report each created worklog id back to the user.

   Worklogs are booked against the issue's **Tempo Account** (billing bucket).
   If the user needs to change which account an issue bills to, `loom tempo
   accounts --search <text>` finds it and `loom tempo set-account --issue K
   --account <key|id>` sets it (confirm first, same as any write).

## B) Draft the weekly status (read-only — the user pastes it themselves)

Loom has **no write access** to the weekly status, but it can **read from every
app** above. Use that to propose what to write:

- Pull the week's activity across all sources; read the user's recent Confluence
  status pages for tone/structure.
...
- Present the draft for the user to copy in. Iterate if they want it tighter.

## Working style

- Draft, then iterate **with** the user. They decide; they apply (or approve the
  write).
- Show your sources so the user can verify.
- For Tempo specifically: proposal → their hours-per-day → agreement → write.
  Never skip the agreement step.
